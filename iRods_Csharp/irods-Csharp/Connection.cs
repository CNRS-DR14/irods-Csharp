...
namespace irods_Csharp
{
    ...
    internal class Connection : IDisposable
    {
        ...
        public void Connect()
        {
            ...
            serverSocket.Connect(_account.irods_host, _account.irods_port);
            _serverStream = new NetworkStream(serverSocket)
            {
                ReadTimeout = 5000
            };

            Packet<StartupPack_PI> connectionRequest = new Packet<StartupPack_PI>(type: MessageType.CONNECT)
            {
        ...
        public void Secure()
        {
            Packet<sslStartInp_PI> sslRequest = new Packet<sslStartInp_PI>(ApiNumberData.SSL_START_AN)
            {
            ...
            SslStream secureServerStream = new SslStream(_serverStream, false);
            secureServerStream.AuthenticateAsClient(_account.irods_host);
        ...
        {
            Packet<None> authRequest = new Packet<None>(ApiNumberData.AUTH_REQUEST_AN);
            SendPacket(authRequest);

            Packet<authRequestOut_PI> authRequestReply = ReceivePacket<authRequestOut_PI>();

            Packet<authResponseInp_PI> authResponse = new Packet<authResponseInp_PI>(ApiNumberData.AUTH_RESPONSE_AN)
            {
        ...
        public string Pam(string password)
        {
            ...
            Packet<authPlugReqInp_PI> msgHeaderStructClient = new Packet<authPlugReqInp_PI>(ApiNumberData.AUTH_PLUG_REQ_AN)
            {
        ...
        {
            Packet<T> packet = new Packet<T> { MsgHeader = ReceiveMessage<MsgHeader_PI>() };
            ...
            Exception e = new Exception(Table.ApiErrorData[packet.MsgHeader.intInfo]);
